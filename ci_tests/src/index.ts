import {Wallet, getDefaultProvider, BytesLike, utils, providers, BigNumber} from "ethers";
...
async function deployEth(rpc_endpoint: string, chain_id: number) : Promise<ChainDetails> {
    ...
    console.log("Deploying libraries of transceiverStructs and normalizedAmounts");
    ...
    let myObj = {
        "src/libraries/TransceiverStructs.sol:TransceiverStructs" : transceiverStructsAddress,
    ...
    const managerProxyAddress = await ERC1967ProxyFactory.deploy(await managerAddress.address, "0x");
    ...
    const manager = NttManager__factory.connect(await managerProxyAddress.address, signer);

    console.log("Deploy transceiver implementation");
    const WormholetransceiverFactory = new WormholeTransceiver__factory(myObj, signer);
    ...
    const transceiverProxyAddress = await transceiverProxyFactory.deploy(await WormholeTransceiverAddress.address, "0x");
    //result = await endpointProxyAddress.waitForDeployment();
    const transceiver = WormholeTransceiver__factory.connect(await transceiverProxyAddress.address, signer);
...
async function link(chain1: ChainDetails, chain2: ChainDetails){
    ...
    */
    ...
    const manager1 = NttManager__factory.connect(<string>chain1.managerAddress, signer1);
    const manager2 = NttManager__factory.connect(<string>chain2.managerAddress, signer2);

    const transceiver1 = WormholeTransceiver__factory.connect(<string>chain1.transceiverAddress, signer1);
    const transceiver2 = WormholeTransceiver__factory.connect(<string>chain2.transceiverAddress, signer2);

    // Would make sense to store the 'client' with a generalized interface instead of the 
    console.log("Set manager peers");
    result = await manager1.setPeer(chain2.chainId, <BytesLike>addressToBytes(<string>chain2.managerAddress));
    result.wait();
    await delay(2000); // Fixing race condition on nonce. Need to figure out why 'waitForDeployment()' does this?
    result = await manager2.setPeer(chain1.chainId, <BytesLike>addressToBytes(<string>chain1.managerAddress));
    result.wait();

    console.log("Set wormhole Peers")
    result = await transceiver1.setWormholePeer(chain2.chainId, <BytesLike>addressToBytes(<string>chain2.transceiverAddress))
    result.wait();
    await delay(2000); // Fixing race condition on nonce. Need to figure out why 'waitForDeployment()' does this?
    result = await transceiver2.setWormholePeer(chain1.chainId, <BytesLike>addressToBytes(<string>chain1.transceiverAddress))
    result.wait();
    
    console.log("Set inbound limits");
    result = await manager1.setInboundLimit(utils.parseEther("10000"), chain2.chainId);
    result.wait();
    await delay(2000);
    result = await manager2.setInboundLimit(utils.parseEther("10000"), chain1.chainId);
    result.wait();
    ...
    result = await transceiver1.setIsWormholeRelayingEnabled(chain2.chainId, true);
    result.wait();
    result = await transceiver2.setIsWormholeRelayingEnabled(chain1.chainId, true);
    result.wait();
...
async function BackAndForthBaseTest(chain1: ChainDetails, chain2: ChainDetails){
    ...
    var balanceUserBeforeSend1 = await token1.balanceOf("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1");
    result = await manager1["transfer(uint256,uint16,bytes32,bool,bytes)"](amount, chain2.chainId, <BytesLike>addressToBytes(<string>"0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1"), false, "0x01000101"); // No relayer - actually works but don't know how to get info from the spy.
    var txResponse = await result.wait();

    var balanceManagerAfterSend1 = await token1.balanceOf(chain1.managerAddress);
    var balanceUserAfterSend1 = await token1.balanceOf("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1");
    ...
    var balanceBeforeRecv = await token2.balanceOf("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1");
    await receive(txResponse, chain1, chain2);

    var balanceAfterRecv = await token2.balanceOf("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1")
    ...
    var balanceUserBeforeSend2 = await token2.balanceOf("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1");
    result = await manager2["transfer(uint256,uint16,bytes32,bool,bytes)"](amount, chain1.chainId, <BytesLike>addressToBytes(<string>"0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1"), false, "0x01000101"); 
    var txResponse = await result.wait(); 
    await delay(5000);
    console.log("Finish second transfer");

    var balanceManagerAfterSend2 = await token2.balanceOf(chain1.managerAddress);
    var balanceUserAfterSend2 = await token2.balanceOf("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1");
    ...
    var balanceBeforeRecv = await token1.balanceOf("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1");
    await receive(txResponse, chain2, chain1);
    console.log("Finish second receieve");

    await delay(500);
    var balanceAfterRecv = await token1.balanceOf("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1");
...
// Relayer base calls
async function BackAndForthBaseRelayertest(chain1: ChainDetails, chain2: ChainDetails){
    ...
    var balanceUserBeforeSend = await token2.balanceOf("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1");
    result = await manager1["transfer(uint256,uint16,bytes32,bool,bytes)"](amount, chain2.chainId, <BytesLike>addressToBytes("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1"), false, "0x01000100", {value: utils.parseEther('1')}); // with relayer
    result.wait();
    // Wait for the relaying and VAA process to pick this up and transmit it.
    await delay(10000);

    var balanceUserAfterSend = await token2.balanceOf("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1");
    ...
    var balanceUserBeforeSend = await token1.balanceOf("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1");
    result = await manager2["transfer(uint256,uint16,bytes32,bool,bytes)"](amount, chain1.chainId, <BytesLike>addressToBytes("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1"), false, "0x01000100", {value: utils.parseEther('1')}); // with relayer

    // Wait for the relaying and VAA process to pick this up and transmit it.
    await delay(10000);

    var balanceUserAfterSend = await token1.balanceOf("0x90F8bf6A479f320ead074411a4B0e7944Ea8c9C1");
    if(!balanceUserBeforeSend.add(amount).eq(balanceUserAfterSend)){
...
async function receive(txResponse, chainSend, chainDest){
    ...
    const transceiver = WormholeTransceiver__factory.connect(<string>chainDest.transceiverAddress, signer);
