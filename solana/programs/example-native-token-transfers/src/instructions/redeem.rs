...
pub fn redeem<'info>(
    ...
    // let (transceiver_message, mut inbox_item, inbox_item_bump) = verify_redeem_accs(
    //     &accs.config,
    //     &accs.transceiver_message,
    //     &accs.transceiver,
    //     &accs.peer,
    //     &accs.inbox_item,
    //     &accs.inbox_rate_limit,
    // )?;
    let (accs, transceiver_message, inbox_item_bump) = ctx.accounts.verify_redeem_accs()?;
    let mut inbox_item: Account<'_, InboxItem> = Account::try_from(&mut accs.inbox_item)?;
