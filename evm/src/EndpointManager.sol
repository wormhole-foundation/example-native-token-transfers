    ...
    ReentrancyGuard
{
    ...
    function transfer(
        ...
        uint256[] memory endpointQuotes = new uint256[](
        ...
        for (uint256 i = 0; i < enabledEndpoints.length; i++) {
            IEndpoint(enabledEndpoints[i]).sendMessage{
                value: endpointQuotes[i]
            }(recipientChain, encodedManagerPayload);
        }
    ...
    function setEndpoint(address endpoint) external onlyOwner {
        ...
        uint64 updatedEnabledEndpointBitmap = enabledEndpointBitmap |
    ...
    function removeEndpoint(address endpoint) external onlyOwner {
        ...
        for (uint256 i = 0; i < enabledEndpoints.length; i++) {
            if (enabledEndpoints[i] == endpoint) {
                enabledEndpoints[i] = enabledEndpoints[
    ...
    function _checkEndpointInvariants(address endpoint) internal view {
        ...
        bool endpointInEnabledBitmap = (enabledEndpointBitmap &
