...
case "$chain_name" in
    ethereum)
        ...
        NTT_CONTRACT=0xDb55492d7190D1baE8ACbE03911C4E3E7426870c
    ...
    arbitrum)
        ...
        NTT_CONTRACT=0xD1a8AB69e00266e8B791a15BC47514153A5045a6
    ...
    optimism)
        ...
        NTT_CONTRACT=0x9bD8b7b527CA4e6738cBDaBdF51C22466756073d
...
# Step 1) Query owner and pauser for the current NTT Transceiver contract (should not be the governance contract)
echo "STEP 1:"
echo "Getting owner and pauser for NTT Transceiver..."
owner=$(cast call "$NTT_CONTRACT" "owner()(address)" --rpc-url "$RPC")
pauser=$(cast call "$NTT_CONTRACT" "pauser()(address)" --rpc-url "$RPC")
if [[ $owner != "$GOV_CONTRACT" ]] || [[ $pauser != "0x${USER_ADDRESS}" ]]; then
  echo "ERROR! Owner is $owner and pauser is $pauser, which is unexpected! Exiting..."
  clean_up
else
  printf "Verified owner is $owner as expected and pauser is $pauser as expected\n\n"
fi

# Step 2) Transfer pauser of the NTT Transceiver contract to the Governance contract
echo "STEP 2:"
...
# # Step 3) Query owner and pauser of the NTT Transceiver contract (should be the governance contract)
echo "STEP 3:"
echo "Getting owner and pauser for NTT Transceiver (should both be "$GOV_CONTRACT")..."
sleep 10
owner=$(cast call "$NTT_CONTRACT" "owner()(address)" --rpc-url "$RPC")
pauser=$(cast call "$NTT_CONTRACT" "pauser()(address)" --rpc-url "$RPC")
