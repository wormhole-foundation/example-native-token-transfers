...
for chain in $operating_chains; do
  ...
  export governance_contract_address=$(jq --raw-output ".GovernanceContracts[] | select(.chainId == $chain) | .address" $contracts_file_path)
  ...
  # echo "governance_contract_address: $governance_contract_address"
