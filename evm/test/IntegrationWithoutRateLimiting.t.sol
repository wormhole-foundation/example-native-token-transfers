...
import "../src/NttManager/NttManagerWithoutRateLimiting.sol";
...
contract TestEndToEndNoRateLimiting is Test {
    NttManagerWithoutRateLimiting nttManagerChain1;
    NttManagerWithoutRateLimiting nttManagerChain2;
    ...
    function setUp() public {
        ...
        NttManagerWithoutRateLimiting implementation = new MockNttManagerWithoutRateLimitingContract(
            address(t1), IManagerBase.Mode.LOCKING, chainId1, 1 days, false
        );

        nttManagerChain1 = MockNttManagerWithoutRateLimitingContract(
        ...
        NttManagerWithoutRateLimiting implementationChain2 = new MockNttManagerWithoutRateLimitingContract(
            address(t2), IManagerBase.Mode.BURNING, chainId2, 1 days, false
        );

        nttManagerChain2 = MockNttManagerWithoutRateLimitingContract(
    ...
    function test_chainToChainBase() public {
        ...
        wormholeTransceiverChain1.receiveMessage(encodedVMs[0]);
        {
            ...
            require(
                ...
                "NttManagerWithoutRateLimiting has unintended funds"
            );
        }
        ...
        // Supply checks on the transfer
        {
            ...
            require(
                token2.balanceOf(address(nttManagerChain2)) == 0,
                "NttManagerWithoutRateLimiting didn't receive unintended funds"
            );
        }
    ...
    function test_multiTransceiver() public {
        ...
        // Send token through standard means (not relayer)
        {
            ...
            require(
                ...
                "NttManagerWithoutRateLimiting should burn all tranferred tokens"
            );
        }
