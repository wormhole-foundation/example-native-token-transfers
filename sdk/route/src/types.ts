...
export namespace MultiTokenNttRoute {
  ...
  export type TokenConfig = {
    chain: Chain;
    // TODO: multi token ntt supports arbitrary tokens,
    // we shouldn't have to configure them here
    // we're just doing it for now for simplicity and Connect doesn't support arbitrary tokens
    token: string;
    manager: string; // MultiTokenNtt contract
    gmpManager: string;
    transceiver: TransceiverConfig[];
    quoter?: string;
  };
  ...
  export type AutomaticAttestationReceipt = {
    id: WormholeMessageId;
    attestation: VAA<"Ntt:MultiTokenWormholeTransferStandardRelayer">;
  };
